...
TGraph *GraphWW() {
  ...
  // Manually add points
  x[0] =  -3.9e-6;    y[0] = 0; // limit on a0W
  x[1] =  3.9e-6;     y[1] = 0; // limit on a0W
  x[2] =  0;          y[2] = -1.4e-5; // limit on aCW
  x[3] =  0;          y[3] = 1.4e-5; // limit on aCW
...
TGraph *GraphZZ() {
  ...
  // Manually add points
  x[0] =  -1e-5;    y[0] = 0; // limit on a0W
  x[1] =  1e-5;     y[1] = 0; // limit on a0W
  x[2] =  0;          y[2] = -4.46e-5; // limit on aCW
  x[3] =  0;          y[3] = 4.46e-5; // limit on aCW
  x[4] =  6.239e-6;   y[4] = 2.2e-5;
  x[5] =  -1.724e-5;  y[5] = 2.2e-5;
  x[6] =  1.723e-5;   y[6] = -2.2e-5;
  x[7] =  -6.225e-6;  y[7] = -2.2e-5;
  x[8] =  -1.439e-6;   y[8] = 5e-5;
  x[9] =  -2.362e-5;  y[9] = 5e-5;
...
void FitEllipse(){
  fitEllipseTGraphDLSF(GraphWW());
  GraphWW_realPoints()->Draw("Psame");
  TCanvas *c = ((TCanvas *)(gROOT->GetListOfCanvases()->FindObject("c")));
  c->Print("Ellipse_ww.pdf");
  fitEllipseTGraphDLSF(GraphZZ());
  GraphZZ_realPoints()->Draw("Psame");
  c = ((TCanvas *)(gROOT->GetListOfCanvases()->FindObject("c")));
  c->Print("Ellipse_zz.pdf");
