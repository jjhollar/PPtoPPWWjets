...
void CMS_lumi(TPad *pad, int iPeriod, int iPosX, TString periodAndLumi,bool twoLine = false) {
  ...
  if (outOfFrame) {
    ...
    latex.DrawLatex(l, 1 - t + lumiTextOffset * t, cmsText+" #scale[0.76]{"+periodAndLumi+"}");
    if (writeExtraText) {
      ...
      latex.DrawLatex(l + 
                       // 0.12, // for square plots
                       0.09, // for rectangular plots
                      1 - t + lumiTextOffset * t,
                      extraText);
    }
  }
  ...
  if (!outOfFrame) {
    if (drawLogo) {
      ...
      if (writeExtraText) {
        ...
        latex.SetTextSize(extraTextSize * t);
        // latex.DrawLatex(posX_, posY_- relExtraDY*cmsTextSize*t, extraText);
        latex.DrawLatex(posX_ + 0.07,
                        posY_ - (cmsTextSize - extraTextSize) * t / relExtraDY,
                        extraText);
      }
    }
  } else if (writeExtraText && !outOfFrame) {
    if (iPosX == 0) {
      posX_ = l + relPosX * (1 - l - r);
...
void CMS_TOTEM_lumi(TPad *pad, int iPeriod, int iPosX, TString periodAndLumi,bool twoLine = false) {
  ...
  if (!outOfFrame) {
    if (drawLogo) {
      ...
      latex.SetTextSize(cmsTextSize * t);
      latex.SetTextAlign(align_);
      cout << "Drawing here at " << posX_ << " " << posY_ << endl;
      latex.DrawLatex(posX_, posY_, cmsText);
      if (writeExtraText) {
        latex.SetTextFont(extraTextFont);
        latex.SetTextAlign(align_);
